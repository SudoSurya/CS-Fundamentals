...
# Define a function to check if a game is possible
def is_possible(game):
    cube_counts = {'red': 0, 'green': 0, 'blue': 0}

    # Count the cubes in each subset of the game
    ...
    # Check if the counts match the target counts
    return all(cube_counts[color] <= target_counts[color] for color in target_counts)
...
# Initialize the sum of IDs
sum_of_ids = 0

# Iterate through games and check if they are possible
for i, game in enumerate(games_data, start=1):
    if is_possible(game):
        sum_of_ids += i

# Print the result
print("The sum of IDs of possible games is:", sum_of_ids)
